...
`primary=done`은 읽을 수 있는 `kind=primary` 출처를 연결해야 한다. 사용자 제공 원본도 원자료로 사용할 수 있다. 원자료 접근이 실패하면 `primary=limited`로 표시한다. `breadth`·`primary`를 해당 없음으로 처리하지 않는다. 다른 관점의 `not_applicable`은 왜 해당 검토가 판단에 영향을 주지 않는지 구체적으로 설명한 경우만 사용한다.
...
`refuted`는 읽을 수 있는 `relation=refutes` 근거가 필요하다. 직접 계산·실험·논리적 반증도 실제 실행 결과나 증명 노트를 출처로 연결한다. 근거 부족은 `refuted`가 아니라 `unresolved`로 구분한다. 이는 연결 검사이며 반증 내용의 타당성은 별도로 검토한다.
