...
- 의미 입력은 실제 토크나이저와 모델의 입력 한도(특수 토큰 포함)로 다시 나눕니다. 각 구간의 벡터를 따로 보존하고 최고 구간 유사도로 검색하므로, 긴 문서의 뒤쪽을 조용히 버리지 않습니다. 결과에는 근거 위치와 일치 구간을 표시합니다.
- `evidence_file`의 UTF-8 근거 노트 본문도 `kind=evidence`로 전역 색인합니다. 지원하지 않는 형식·인코딩은 원본을 보존하고 `coverage[].unindexed_evidence`에 표시합니다.
- `refuted` 주장은 읽을 수 있는 반박 근거를, `primary=done`은 읽을 수 있는 원자료를 연결해야 합니다. 이 검사는 기록 일관성만 확인하며 의미상 반증을 보증하지 않습니다.
- 전역 DB는 첫 쓰기에 스키마 3으로 이전합니다. 원본 연구와 기존 벡터는 이전 중 보존되지만 구버전 벡터는 새 검색 준비 상태로 계산하지 않습니다. `refresh --semantic required`로 새 토큰 구간을 구축해야 합니다. 이전 전 원본과 DB를 백업하세요.

오프라인 회귀시험과 별도로 실제 고정 모델을 사용하는 소규모 검색 검사를 제공합니다. 이는 가상 자료를 사용하는 기능 확인이며 일반적인 검색 품질 벤치마크가 아닙니다. 명시적으로 다운로드를 허용한 격리 환경에서 실행합니다.

```bash
python3 evals/real_model_smoke.py --allow-download --output /tmp/research-semantic-smoke.json
```
...
1.3.0/1.4.0 DB는 첫 쓰기 시 스키마 3으로 이전하며 기존 키·벡터를 보존합니다. 새 구간 색인에 맞지 않는 벡터는 재계산 전까지 검색에서 제외합니다. 이전 전 원본과 DB 사본을 보관하고, 복구 시 스킬과 DB 버전을 함께 되돌리세요. `local/off` 정책은 전역 색인 기여를 제거하며 원본 연구를 삭제하지 않습니다. 별도 백업에는 같은 보존 결정을 적용해야 합니다.
